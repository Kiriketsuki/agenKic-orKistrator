...
func TestFullLifecycle(t *testing.T) {
	...
	// --- SpawnSession ---
	sess, err := sub.SpawnSession(ctx, sessionName, "bash", SessionOptions{})
	if err != nil {
...
func TestDuplicateSession(t *testing.T) {
	...
	_, err = sub.SpawnSession(ctx, sessionName, "", SessionOptions{})
...
func TestMultiPaneGrid(t *testing.T) {
	...
	// Verify via ListSessions that WindowCount is reasonable.
	sessions, err := sub.ListSessions(ctx)
	if err != nil {
		t.Fatalf("ListSessions: %v", err)
	}
